 ...
 * @type {module:fs}
 */
let echo = require("debug")('app:testPoemApi');
...
const HOME_API = 'http://kan.msxiaobing.com/ImageGame/Portal?task=poem';
const FACE_API = 'http://kan.msxiaobing.com/Api/ImageAnalyze/Process?service=poem';
...
function saveData(data, msgId) {
    let key = wxConstant.POEM_REDIS_PREFIX + msgId;
    let expire = wxConstant.POEM_REDIS_EXPIRE;

    redisUtil.set(key, JSON.stringify(data), (err) => {
        if (err) {
            echo('redis存储[key=' + key + ']作诗数据失败。');
        }
        echo('redis存储[key=' + key + ']作诗数据成功。');
    });
