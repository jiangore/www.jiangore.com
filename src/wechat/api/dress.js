 ...
 * @type {module:fs}
 */
let echo = require("debug")('app:testDressApi');
...
const HOME_API = 'http://kan.msxiaobing.com/ImageGame/Portal?task=cosmoclothing';
const FACE_API = 'http://kan.msxiaobing.com/Api/ImageAnalyze/Process?service=cosmoclothing';
    ...
    init() {
        image2Base64(this.image).then(data => {
            ...
            let data = {
                code: '1',
                title: '时尚穿衣秘籍',
                description: content.metadata.Comments_Text0 + '\n' + content.metadata.Comments_Text1 + '\n' + content.metadata.Comments_Text2,
                picUrl: this.image,
...
function saveData(data, msgId) {
    let key = wxConstant.DRESS_REDIS_PREFIX + msgId;
    let expire = wxConstant.DRESS_REDIS_EXPIRE;

    redisUtil.set(key, JSON.stringify(data), (err) => {
        if (err) {
            echo('redis存储[key=' + key + ']穿衣数据失败。');
        }
        echo('redis存储[key=' + key + ']穿衣数据成功。');
    });
