 ...
 * @type {module:fs}
 */
let echo = require("debug")('app:testBeautyApi');
...
const HOME_API = 'http://kan.msxiaobing.com/ImageGame/Portal?task=beauty';
const FACE_API = 'http://kan.msxiaobing.com/Api/ImageAnalyze/Process?service=beauty';
    ...
    init() {
        image2Base64(this.image).then(data => {
            ...
            let data = {
                ...
                picUrl: content.imageUrl,
                url: content.metadata.reportImgUrl
            };
...
function saveData(data, msgId) {
    ...
    redisUtil.set(key, JSON.stringify(data), (err) => {
        if (err) {
            echo('redis存储[key=' + key + ']颜值鉴定数据失败。');
        }
        echo('redis存储[key=' + key + ']颜值鉴定数据成功。');
    });
